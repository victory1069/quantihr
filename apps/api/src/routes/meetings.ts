...
export function registerMeetingRoutes(app: FastifyInstance, db: Database): void {
  ...
  app.post('/v1/meetings/:id/checkin', async (request, reply) => {
    const auth = requireAuth(request)
    const { id } = request.params as { id: string }
    const body = schemas.meetings.roomCheckIn.parse(request.body)
    ...
    await tenant(request, async (tx) => {
      ...
      // Verify the code. This previously accepted anything of the right shape,
      // which made the room code decorative — anyone with the meeting id could
      // put themselves in the room. As with the door code it is a deterrent and
      // a record, not proof of presence, but it has to be the actual code.
      if (!meeting.checkinCode) {
        throw new ApiError(
          ERROR_CODES.CHECKIN_CODE_INVALID,
          'No check-in code has been generated for this meeting yet',
          409,
        )
      }
      if (body.code.trim().toUpperCase() !== meeting.checkinCode) {
        throw new ApiError(ERROR_CODES.CHECKIN_CODE_INVALID, 'That code is not right', 422)
      }
      if (meeting.checkinCodeExpiresAt && meeting.checkinCodeExpiresAt < new Date()) {
        throw new ApiError(
          ERROR_CODES.CHECKIN_CODE_STALE,
          'That code has expired. Ask the host for the current one.',
          422,
        )
      }
  ...
  /**
   * Meeting check-in codes.
   *
   * Separate from the door code on purpose. The door code is a rotating,
   * per-location secret that HR puts on a screen at the entrance; this is a
   * fixed code for one sitting, generated by whoever is running that meeting.
   *
   * **Who can generate one.** HR admins and owners for any meeting, and the
   * host for their own. Restricting it to HR alone would mean a manager cannot
   * start their own meeting without ringing someone, which is how a feature
   * stops being used.
   *
   * The code does not rotate mid-meeting. A rotation partway through would lock
   * out whoever arrived late, and that is exactly the person who still needs to
   * check in.
   */
  app.post('/v1/meetings/:id/checkin-code', async (request, reply) => {
    const auth = requireAuth(request)
    const { id } = request.params as { id: string }
    const body = schemas.meetings.generateMeetingCode.parse(request.body ?? {})

    const result = await tenant(request, async (tx) => {
      const meeting = await loadMeeting(tx, id)
      if (meeting.hostEmployeeId !== auth.employeeId && !isHrAdmin(auth)) {
        throw new ApiError(
          ERROR_CODES.MEETING_NOT_HOST,
          'Only the host or an HR admin can generate a code for this meeting',
          403,
        )
      }
      ...
      // Unambiguous alphabet: no O/0, no I/1. Someone is reading this off a
      // screen across a room and typing it on a phone.
      ...
      await audit(tx, {
        orgId: auth.orgId,
        actorUserId: auth.userId,
        action: 'meeting.checkin_code_issued',
        entityType: 'meeting',
        entityId: id,
        after: { venue: updated!.venue, expiresAt: expiresAt.toISOString() },
        ip: request.ip,
      })
    ...
    return reply.send(await codeView(request, result))
  })

  /** Codes for meetings that are running or about to. HR and hosts. */
  app.get('/v1/meetings/checkin-codes', async (request, reply) => {
    const auth = requireAuth(request)

    const rows = await tenant(request, async (tx) => {
      const now = new Date()
      const window = new Date(now.getTime() + 12 * 60 * 60 * 1000)
      ...
      const found = await tx
        ...
        .where(
          and(
            eq(meetings.source, 'in_person'),
            gte(meetings.scheduledStart, from),
            lt(meetings.scheduledStart, window),
          ),
        )
        .orderBy(meetings.scheduledStart)

      // HR sees every in-person sitting; a host sees their own. A manager with
      // no meetings today gets an empty list rather than a permission error.
      const visible = isHrAdmin(auth)
        ? found
        : found.filter((m) => m.hostEmployeeId === auth.employeeId)

      return Promise.all(visible.map((m) => codeViewTx(tx, m)))
    })
...
/** One meeting rendered for the code screens, with its live check-in count. */
async function codeViewTx(tx: Tx, meeting: typeof meetings.$inferSelect) {
  const [counted] = await tx
    ...
    .where(
      and(
        ...
        eq(meetingParticipants.source, 'in_person'),
      ),
    )

  const [location] = meeting.locationId
    ? await tx
  ...
  return {
    ...
    expiresAt: meeting.checkinCodeExpiresAt?.toISOString() ?? null,
    venue: meeting.venue,
    agenda: meeting.agenda,
    locationName: location?.name ?? null,
    scheduledStart: meeting.scheduledStart.toISOString(),
    checkedIn: counted?.n ?? 0,
  }
}

const codeView = (
  request: Parameters<typeof tenant>[0],
  meeting: typeof meetings.$inferSelect,
) => tenant(request, async (tx) => codeViewTx(tx, meeting))
